...
# Safety DDI content is ~10k chars per pair (full FDA sections for both drugs). 15 pairs × 10k = 150k → exceeds Azure ~100k content
...
class VabGenRxSafetyAgent(_BaseAgent):
    ...
    def synthesize(
        ...
        def _run_ddi_batch_with_retry(batch, meds_str, batch_num):
            """
            Run a DDI batch — with two optimizations:
