...
import { ScanLine, Save, CheckCircle2, AlertCircle, RotateCcw, ClipboardList, FileDown, Zap, X } from "lucide-react";
...
const DiagnosisTab = ({ p, user }) => {
    ...
    const save = async () => {
      try {
        await apiFetch(isOutpatient ? "/api/op-patient-counselling" : "/api/ip-patient-counselling", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...(isOutpatient ? { op_no: patientNo } : { ip_no: patientNo }), drug_counselling: drugC, condition_counselling: condC }),
        });
      } catch (err) { console.error(err); }
    };
    save();
  ...
  const checkStockForMed = async (med) => {
    if (!med?.Generic_Name || !med?.Strength) return;
    try {
      const res  = await apiFetch(`/api/drug-inventory/search?q=${encodeURIComponent(med.Generic_Name.trim())}`);
      const data = await res.json();
      if (!res.ok) return;
      const sameStrength = (data.drugs || []).filter(d =>
        d.Generic_Name?.toLowerCase().trim() === med.Generic_Name.toLowerCase().trim() &&
        d.Strength?.trim() === med.Strength?.trim()
      );
      const outEntry = sameStrength.find(d => d.Brand_Name === med.Brand_Name && (d.Stocks === 0 || d.Stocks === "0" || !d.Stocks));
      if (!outEntry) return;
      const alternatives = sameStrength.filter(d => d.Brand_Name !== med.Brand_Name && parseInt(d.Stocks) > 0);
      const presRes  = await apiFetch(isOutpatient ? `/api/op-prescriptions/${encodeURIComponent(patientNo)}` : `/api/ip-prescriptions/${encodeURIComponent(patientNo)}`);
      ...
      if (!presRow) return;
      setOutOfStock(prev => prev.find(o => o.med.Brand_Name === med.Brand_Name) ? prev : [...prev, { med: { ...med, ID: presRow.ID }, alternatives }]);
    } catch {}
  };
  ...
  const handleSwitch = async (outMed, altDrug) => {
    try {
      await apiFetch(isOutpatient ? "/api/op-prescriptions/delete" : "/api/ip-prescriptions/delete", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: outMed.ID }),
      });
  ...
  const handleSaveNote = async () => {
    ...
    try {
      const res = await apiFetch(isOutpatient ? "/api/op-prescription-notes" : "/api/ip-prescription-notes", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(isOutpatient ? { opNo: patientNo, notes: noteText.trim() } : { ipNo: patientNo, notes: noteText.trim() }),
      });
  ...
  const handleSaveNoteEdit = async (id) => {
    ...
    try {
      await apiFetch(isOutpatient ? "/api/op-prescription-notes/update" : "/api/ip-prescription-notes/update", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, notes: editNoteText.trim() }),
      });
  ...
  // ✅ Auto-fill Route from drug_inventory when drug is selected
  ...
  const handleHold = async (id) => {
    ...
    try {
      await apiFetch(isOutpatient ? "/api/op-prescriptions/hold" : "/api/ip-prescriptions/hold", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, held: newHeld }),
      });
    } catch (err) { console.error(err); }
  };
        ...
        <div className="presc-overlay" onClick={() => setShowPrescribe(false)}>
          <div className="presc-modal" onClick={e => e.stopPropagation()}>
            <div className="presc-header">
              ...
              <button className="presc-close" onClick={() => setShowPrescribe(false)}>
                <X size={14} />
              </button>
            </div>
