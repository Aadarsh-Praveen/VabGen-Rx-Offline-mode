...
const PatientCounselling = ({ agentResult, agentLoading, counselTab, setCounselTab, p }) => {
  ...
  const totalPoints =
    ...
    conditionCounseling.reduce((acc, c) =>
      acc + (c.exercise?.length||0)+(c.diet?.length||0)+(c.lifestyle?.length||0)+(c.safety?.length||0), 0);
  ...
  // ── Deny / Retrieve handlers ─────────────────────────────────
  const handleDeny     = (key) => setDenied(p => ({ ...p, [key]: true }));
  const handleRetrieve = (key) => setDenied(p => { const n = { ...p }; delete n[key]; return n; });
  ...
  const handleSavePdf = () => {
    ...
    const drug = (translated?.drugData || approvedDrugData).map(d => ({
      ...d,
    ...
    const badge = (level) => {
      if (!level) return "";
      const s = level === "high"
        ? "background:#fff0f0;color:#c0392b;border:1px solid #fca5a5;"
        : level === "medium"
        ? "background:#fff7ed;color:#d97706;border:1px solid #fcd34d;"
    ...
    const icon = (cat) => {
      const m = { bleeding:"🩸", monitoring:"🔬", timing:"⏰", renal:"🫘", cardiac:"❤️" };
      return m[cat] || "⚠️";
    };
      ...
      drug.forEach(d => {
        ...
        (d.points || []).forEach(pt => {
          ...
          body += `
            <div class="item">
              <div class="item-top">
                <span class="item-icon">${icon(pt.category)}</span>
                <strong class="item-title">${esc(pt.title)}</strong>
                ${badge(bl)}
              </div>
              <p class="item-desc">${esc(pt.detail)}</p>
            </div>`;
        });
      });
    }
    ...
    if (cond.length > 0) {
      ...
      cond.forEach(c => {
        ...
        [
          { label:"🏃 Exercise",  items: c.exercise  },
          { label:"🥗 Diet",      items: c.diet      },
          { label:"🌿 Lifestyle", items: c.lifestyle },
          { label:"⚠️ Safety",   items: c.safety    },
        ].forEach(sec => {
          if (!sec.items?.length) return;
          body += `<p class="sub-label">${esc(sec.label)}</p>`;
          sec.items.forEach(it => {
            const detail = it.frequency
              ? `${esc(it.detail)} — 📅 ${esc(it.frequency)}`
              : esc(it.detail || "");
            body += `
              <div class="item">
                <div class="item-top">
                  <strong class="item-title">${esc(it.title)}</strong>
                  ${badge(it.urgency)}
                </div>
                <p class="item-desc">${detail}</p>
              </div>`;
          });
        });
      });
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
    body {
      font-family: "Noto Sans SC","Noto Sans JP","Noto Sans KR","Noto Sans Arabic","Noto Sans Devanagari","Noto Sans Tamil","Noto Sans Telugu","Noto Sans Bengali","Noto Sans Malayalam","Noto Sans Kannada","Noto Sans",Arial,sans-serif;
      font-size:13px; color:#000; background:#fff; padding:32px;
      ${isRtl ? "direction:rtl;text-align:right;" : ""}
    }
    .header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:24px; padding-bottom:14px; border-bottom:2.5px solid #1a73e8; }
    .logo   { font-size:17px; font-weight:800; color:#1a73e8; }
    .meta   { text-align:${isRtl?"left":"right"}; font-size:11px; line-height:1.9; color:#333; }
    .meta strong { font-weight:700; }
    .sec-title { font-size:14px; font-weight:700; color:#1a73e8; margin:22px 0 10px; padding-bottom:5px; border-bottom:1px solid #e0e3ef; }
    .drug-name { font-size:12px; font-weight:700; color:#111; margin:12px 0 6px; }
    .sub-label { font-size:10px; font-weight:700; color:#777; text-transform:uppercase; letter-spacing:0.05em; margin:10px 0 4px; }
    .item { background:#f8f7ff; border-left:${isRtl?"none":"3px solid #1a73e8"}; border-right:${isRtl?"3px solid #1a73e8":"none"}; padding:8px 12px; margin-bottom:6px; border-radius:5px; page-break-inside:avoid; }
    .item-top { display:flex; align-items:center; flex-wrap:wrap; gap:6px; margin-bottom:4px; }
    .item-icon  { font-size:13px; flex-shrink:0; }
    .item-title { font-size:12px; font-weight:700; color:#111; }
    .item-desc  { font-size:11px; color:#333; line-height:1.65; margin-top:2px; }
    .footer { margin-top:36px; padding-top:12px; border-top:1px solid #e0e3ef; display:flex; justify-content:space-between; font-size:10px; color:#888; }
...
<body>
  <div class="header">
    <div class="logo">🩺 VabGenRx — Patient Counselling</div>
    <div class="meta">
      ${p?.Name ? `<div><strong>Patient:</strong> ${esc(p.Name)}</div>` : ""}
      ${p?.Age  ? `<div><strong>Age:</strong> ${esc(String(p.Age))}</div>` : ""}
      ${p?.Sex  ? `<div><strong>Sex:</strong> ${p.Sex === "M" ? "Male" : "Female"}</div>` : ""}
      <div><strong>Date:</strong> ${date}</div>
      ${selectedLang !== "English" ? `<div><strong>Language:</strong> ${esc(selectedLang)}</div>` : ""}
    </div>
  </div>
  ${body}
  <div class="footer">
    ...
    if (!printWin) {
      ...
      const doIframePrint = () => {
        ...
        setTimeout(() => { if (document.body.contains(iframe)) document.body.removeChild(iframe); setPdfGenerating(false); }, 1500);
      };
      iframe.onload = () => setTimeout(doIframePrint, 1200);
      setTimeout(() => { if (document.body.contains(iframe)) doIframePrint(); }, 3500);
      return;
    }
      ...
      {showPreview && (
        <div className="pcoun-modal-overlay" onClick={() => setShowPreview(false)}>
          <div className="pcoun-modal" onClick={e => e.stopPropagation()}>
            ...
            {/* Modal body — only non-denied rows */}
