  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const ip = (ipRes.ok && ipData.patients) ? ipData.patients : [];
        const op = (opRes.ok && opData.patients) ? opData.patients : [];
        const combined = [
          ...ip.map(p => ({ ...p, _type: "IP", _no: p.IP_No })),
          ...op.map(p => ({ ...p, _type: "OP", _no: p.OP_No })),
        ].slice(0, 5);
        setAppointments(combined.map((p, i) => ({
          ...
          <div className="dash-left-col">
            ...
            <div className="dash-panel dash-appointments">
              ...
              <table className="appt-table">
                <thead>
                  <tr>{["PATIENT", "TIME", "TYPE", "STATUS", "DEPT", "ACTIONS"].map(h => <th key={h}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {appointments.length === 0 ? (
                    ...
                    return (
                      <tr key={a.id} style={{ opacity: isHeld ? 0.7 : 1, background: isHeld ? '#f8fafc' : '', filter: isHeld ? 'grayscale(40%)' : 'none', transition: 'all 0.3s ease', position: 'relative' }}>
                        <td>
                          <div className="appt-patient">
                            ...
                            <div>
                              ...
                              <p className="appt-id">
                                {a.id}
                                <span style={{ marginLeft: '6px', fontSize: '10px', fontWeight: 600, padding: '1px 6px', borderRadius: '9999px', background: a.patientType === 'IP' ? '#dbeafe' : '#dcfce7', color: a.patientType === 'IP' ? '#1d4ed8' : '#166534' }}>{a.patientType}</span>
                                {isHeld && <span style={{ marginLeft: '6px', fontSize: '10px', fontWeight: 700, padding: '1px 8px', borderRadius: '9999px', background: '#fef3c7', color: '#92400e', border: '1px solid #fcd34d' }}>ON HOLD</span>}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="appt-actions" style={{ opacity: 1, filter: 'none' }}>
                          <div style={{ position: 'relative', display: 'inline-block' }} ref={el => menuRef.current[a.id] = el}>
                            <button
                              ...
                              style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #e5e7eb', background: openMenu === a.id ? '#f3f4f6' : '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', color: '#6b7280', transition: 'background 0.15s' }}
                            >⋮</button>

                            {openMenu === a.id && (
                              <div style={{ position: 'absolute', right: 0, top: '38px', background: '#ffffff', borderRadius: '10px', boxShadow: '0 12px 36px rgba(0,0,0,0.16)', border: '1.5px solid #e2e8f0', minWidth: '165px', zIndex: 999, overflow: 'hidden', opacity: 1, filter: 'none', isolation: 'isolate' }}>

                                <button style={viewBtn.base} onMouseEnter={viewBtn.enter} onMouseLeave={viewBtn.leave} onClick={() => { setOpenMenu(null); navigate(`/patients/${a.id}`); }}>
                                ...
                                {!isHeld ? (
                                  <button style={holdBtn.base} onMouseEnter={holdBtn.enter} onMouseLeave={holdBtn.leave} onClick={() => handleHold(a.id)}>
                                  ...
                                  <>
                                    <button style={notesBtn.base} onMouseEnter={notesBtn.enter} onMouseLeave={notesBtn.leave} onClick={() => { openNoteModal(a.id); setOpenMenu(null); }}>
                                    ...
                                    <button style={retrieveBtn.base} onMouseEnter={retrieveBtn.enter} onMouseLeave={retrieveBtn.leave} onClick={() => { handleRetrieve(a.id); setOpenMenu(null); }}>
