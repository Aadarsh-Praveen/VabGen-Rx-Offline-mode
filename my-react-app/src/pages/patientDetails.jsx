...
const ReferralTab = ({ p, isOutpatient }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) { setSaveMsg("success"); setReferral({ to_dept: "", to_doctor: "", urgency: "Routine", reason: "", notes: "", date: new Date().toISOString().split("T")[0] }); await fetchReferrals(); }
...
// ── Tabs Config ───────────────────────────────────────────────
const TABS = [
  { key: "info",      label: "Patient Information",    Icon: UserIcon        },
  { key: "lab",       label: "Lab Results",            Icon: FlaskIcon       },
  { key: "diagnosis", label: "Diagnosis & Prescription", Icon: StethoscopeIcon },
  { key: "referral",  label: "Referral",               Icon: ReferralIcon    },
];
          ...
          <>
            ...
            <div className="pd-tabs">
              {TABS.map(({ key, label, Icon }) => (
                ...
                >
                  <span className="pd-tab-icon"><Icon /></span>
                  {label}
                </button>
              ))}
            </div>
